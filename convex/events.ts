...
import { processQueueX } from "./waiting_list";
  ...
  handler: async (ctx, { eventId, userId }) => {
    ...
    if (available) {
      ...
      const waitingListId = await ctx.db.insert("waitingList", {
        eventId,
        userId,
        status: WAITING_LIST_STATUS.OFFERED ?? "offered", // Mark as offered
      ...
      await ctx.db.insert("waitingList", {
        eventId,
        userId,
        status: WAITING_LIST_STATUS.WAITING ?? "waiting", // Mark as waiting
      });
    }
